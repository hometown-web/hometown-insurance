...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    await sql`INSERT INTO leads (
      full_name, email, phone, insurance_type, other_insurance, other_details,
      contact_method, callback_day, callback_time,
      hear_about, hear_other, promo_code, message,
      business_name, page, form_position
    ) VALUES (
      ${data.full_name}, ${data.email}, ${data.phone || ''},
      ${data.insurance_type || ''}, ${data.other_insurance || ''}, ${data.other_details || ''},
    ...
    try {
      ...
      await resend.emails.send({
        ...
        html: `
          ...
          <p><strong>Insurance Type:</strong> ${data.insurance_type || '—'}${data.other_insurance ? ` (${data.other_insurance})` : ''}${data.other_details ? ` (${data.other_details})` : ''}</p>
          ...
          <p><strong>How They Heard:</strong> ${data.hear_about || '—'}${data.hear_other ? ` (${data.hear_other})` : ''}</p>
